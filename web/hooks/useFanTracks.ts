      ...
      try {
        ...
        const trackPromises = userLogs.map(async (log) => {
          ...
          try {
            ...
            const [artist, trackPrice, currentOwner, uri] = result as [string, bigint, string, string]
            ...
            console.error("Failed to fetch track details for token", tokenId, err)
            return null
          }
        })

        const fetchedTracks = (await Promise.all(trackPromises)).filter((t) => t !== null) as (Track & {
          purchasePrice: bigint
        })[]
        setOwnedTracks(fetchedTracks)

        // Calculate total spent
        const total = fetchedTracks.reduce((sum, track) => sum + track.purchasePrice, 0n)
        setTotalSpent(total)
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : "Failed to fetch owned tracks"
