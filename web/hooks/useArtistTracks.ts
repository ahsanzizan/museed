      ...
      try {
        ...
        const trackPromises = artistLogs.map(async (log) => {
          ...
          try {
            ...
            const [artist, price, currentOwner, uri] = result as [string, bigint, string, string]
        ...
        const total = fetchedTracks.reduce((sum, track) => sum + track.earnings, 0n)
        setTotalEarnings(total)
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : "Failed to fetch artist tracks"
